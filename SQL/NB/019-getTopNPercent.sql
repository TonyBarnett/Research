CREATE FUNCTION getTopNPercentResults(@N float)
...
INNER JOIN (
	SELECT strTrainingMethod, strType, strSystemId, strValue, MAX(fltProbability) * (1 + (@N / CAST(100 AS float))) AS maxProbability
